...
{:else if action instanceof ActionSelectNewChain}
  <div>
    Waiting for <Username username={usernames[action.playerID]} /> to select new chain (<GameStatusHotelInitialsList chains={action.availableChains} />).
  </div>
{:else if action instanceof ActionSelectMergerSurvivor}
  <div>
    Waiting for <Username username={usernames[action.playerID]} /> to select merger survivor (<GameStatusHotelInitialsList chains={action.chainsBySize[0]} />).
  </div>
{:else if action instanceof ActionSelectChainToDisposeOfNext}
  <div>
    Waiting for <Username username={usernames[action.playerID]} /> to select chain to dispose of next (<GameStatusHotelInitialsList
      chains={action.defunctChains}
    />).
  </div>
