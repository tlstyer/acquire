...
<script lang="ts">
  ...
  import HotelInitialsList from './HotelInitialsList.svelte';
...
{#if action instanceof ActionStartGame}
  <div>Waiting for <Username username={usernames[action.playerID]} /> to start the game.</div>
...
{:else if action instanceof ActionSelectNewChain}
  <div>Waiting for <Username username={usernames[action.playerID]} /> to select new chain (<HotelInitialsList chains={action.availableChains} />).</div>
{:else if action instanceof ActionSelectMergerSurvivor}
  <div>Waiting for <Username username={usernames[action.playerID]} /> to select merger survivor (<HotelInitialsList chains={action.chainsBySize[0]} />).</div>
{:else if action instanceof ActionSelectChainToDisposeOfNext}
  <div>
    Waiting for <Username username={usernames[action.playerID]} /> to select chain to dispose of next (<HotelInitialsList chains={action.defunctChains} />).
  </div>
{:else if action instanceof ActionDisposeOfShares}
  <div>
    Waiting for <Username username={usernames[action.playerID]} /> to dispose of <HotelName chain={action.defunctChain} /> shares.
  </div>
{:else if action instanceof ActionPurchaseShares}
  <div>Waiting for <Username username={usernames[action.playerID]} /> to purchase shares.</div>
