...
<script lang="ts">
  ...
  for (let playerID = 0; playerID < usernames.length; playerID++) {
    defaultClassNames[playerID] = isTeamGame ? teamNumberToCSSClassName.get((playerID % numTeams) + 1)! : 'player';
  }
  ...
  <tbody>
    <tr>
      ...
      {#each allChains as chain}
        <td class={gameBoardTypeToCSSClassName.get(chain)}>{gameBoardTypeToHotelInitial.get(chain)}</td>
      {/each}
