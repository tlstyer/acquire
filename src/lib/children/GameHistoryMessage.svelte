...
<script lang="ts">
  ...
  const username = gameHistoryMessageData.playerID !== null ? usernames[gameHistoryMessageData.playerID] : '';
</script>
...
<div>
  ...
  {:else if gameHistoryMessageData.gameHistoryMessage === GameHistoryMessageEnum.DrewPositionTile}
    <Username {username} /> drew position tile {getTileString(gameHistoryMessageData.parameters[0])}.
  ...
  {:else if gameHistoryMessageData.gameHistoryMessage === GameHistoryMessageEnum.SelectedMergerSurvivor}
    <Username {username} /> selected <HotelName chain={gameHistoryMessageData.parameters[0]} /> as merger survivor.
  {:else if gameHistoryMessageData.gameHistoryMessage === GameHistoryMessageEnum.SelectedChainToDisposeOfNext}
    <Username {username} /> selected <HotelName chain={gameHistoryMessageData.parameters[0]} /> as chain to dispose of next.
  {:else if gameHistoryMessageData.gameHistoryMessage === GameHistoryMessageEnum.ReceivedBonus}
    <Username {username} /> received a ${gameHistoryMessageData.parameters[1] * 100} <HotelName chain={gameHistoryMessageData.parameters[0]} /> bonus.
  {:else if gameHistoryMessageData.gameHistoryMessage === GameHistoryMessageEnum.DisposedOfShares}
    <Username {username} /> traded {gameHistoryMessageData.parameters[1]} and sold {gameHistoryMessageData.parameters[2]}
  ...
  {:else if gameHistoryMessageData.gameHistoryMessage === GameHistoryMessageEnum.PurchasedShares}
    ...
    {:else if gameHistoryMessageData.parameters[0].length === 1}
      {gameHistoryMessageData.parameters[0][0][1]} <HotelName chain={gameHistoryMessageData.parameters[0][0][0]} />.
  ...
  {:else if gameHistoryMessageData.gameHistoryMessage === GameHistoryMessageEnum.ReplacedDeadTile}
    <Username {username} /> replaced dead tile {getTileString(gameHistoryMessageData.parameters[0])}.
