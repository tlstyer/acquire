...
<script lang="ts">
  ...
  import { gameModeToNumPlayers, gameModeToString, gameModeToTeamSize, gameStatusToString, teamNumberToCSSClassName } from './helpers';
  ...
  <table>
    <tbody>
      {#each usernames as username, playerID}
        <tr>
          <td class={isTeamGame ? teamNumberToCSSClassName.get((playerID % numTeams) + 1) : 'player'}>{username !== null ? username : ''}</td>
        </tr>
      {/each}
    </tbody>
  </table>
