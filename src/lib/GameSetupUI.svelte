...
<script lang="ts">
  ...
  export let onChangePlayerArrangementMode: ((playerArrangementMode: PB_PlayerArrangementMode) => void) | undefined = undefined;
  export let onSwapPositions: ((position1: number, position2: number) => void) | undefined = undefined;
...
<div>
  ...
  {#if onChangeGameMode !== undefined}
    <select value={gameMode} on:change={(event) => onChangeGameMode?.(parseInt(event.currentTarget.value, 10))}>
      {#each allGameModes as gameMode}
        <option value={gameMode} disabled={(gameModeToNumPlayers.get(gameMode) ?? 0) < numUsersInGame}>
...
<div>
  ...
  {#if onChangePlayerArrangementMode !== undefined}
    <select value={playerArrangementMode} on:change={(event) => onChangePlayerArrangementMode?.(parseInt(event.currentTarget.value, 10))}>
...
<table>
  <tbody>
    ...
    {:else if playerArrangementMode === PB_PlayerArrangementMode.EXACT_ORDER}
      {#each usernames as username, index}
        <tr>
          <td class={isTeamGame ? teamNumberToCSSClassName.get((index % numTeams) + 1) : 'user'}>{username !== null ? username : ''}</td>
          {#if onSwapPositions !== undefined}
            <td>
              {#if index > 0}
                <input type="button" value="▲" on:click={() => onSwapPositions?.(index, index - 1)} />
              {/if}
            </td>
            <td>
