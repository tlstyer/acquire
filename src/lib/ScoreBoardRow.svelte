...
<tr class={defaultClassName}>
  <td class={isPlayersTurn ? 'isPlayersTurn' : isPlayersMove ? 'isPlayersMove' : isPlayerRow ? 'player' : undefined} title={isPlayerRow ? title : undefined}>
    {title}
  </td>
  ...
  {:else}
    ...
    <td class={teamNumber !== undefined ? teamNumberToCSSClassName.get(teamNumber) : 'bottomRightCells'}>
