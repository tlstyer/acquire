                    ...
                    if len(self._result) == 8:
                        ...
                        game_date = int(
                            datetime.datetime(
                                *map(
                                    int,
                                    MyHTMLParser.datetime_regex.match(
                                        game_date
                                    ).groups(),
                                ),
                                tzinfo=pytz.utc
                        ...
                        if (
                            ...
                            and game_type in game_type_to_num_players
                        ):
...
def part1():
    game_type_to_date_to_result = {
        game_type: {} for game_type in game_type_to_mode.keys()
    }
...
def get_game_data():
        ...
        for date, result in sorted(date_to_result.items()):
            ...
            if num_players < num_players_needed:
                ...
                for new_date in range(date - 1, date - 31, -1):
                    if (
                        new_date in bad_date_to_count
                        and bad_date_to_count[new_date] + num_players
                        <= num_players_needed
                    ):
    ...
    for game_type, date_to_result in game_type_to_date_to_result.items():
        date_to_tweaked_dates_and_results = (
            game_type_to_date_to_tweaked_dates_and_results.get(game_type, {})
        )
                ...
                if num_players == num_players_needed:
                    ...
                    if game_type == "teams":
                        if scores[1][1][0] == scores[2][1][0]:
                            has_draw = True
                            scores = sorted(
                                result.items(), key=lambda x: (-x[1][1], x[0].lower())
                            )
    ...
    return {
        "results": results,
        "game_type_to_total_count": game_type_to_total_count,
        "game_type_to_draw_count": game_type_to_draw_count,
    }
...
def part2():
    ...
    for date, game_type, scores in game_data["results"]:
        ...
        print(
            comp,
            date,
            datetime.datetime.fromtimestamp(date, tz=pytz.utc),
            game_type,
            scores,
        )
