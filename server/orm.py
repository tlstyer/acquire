...
class Lookup:
    ...
    def get_game_player(self, game, player_index):
        ...
        if game.game_id:
            game_player = self.session.query(GamePlayer).filter_by(game_id=game.game_id, player_index=player_index).scalar()
    ...
    def get_rating(self, user, rating_type):
        ...
        if user.user_id:
            rating = self.session.query(Rating).filter_by(user=user, rating_type=rating_type).order_by(Rating.rating_id.desc()).limit(1).scalar()
